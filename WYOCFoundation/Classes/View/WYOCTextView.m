...
@interface WYOCTextView ()<UITextViewDelegate>

@property (strong, nonatomic) UIToolbar *toolbar;

@end

@implementation WYOCTextView
...
- (UITextView *)textView {
    if (!_textView) {
        ...
        _textView.textColor = WYOC_COLOR_HEX(0x333333);
        _textView.returnKeyType = UIReturnKeyDone;
        _textView.textAlignment = NSTextAlignmentRight;
        _textView.inputAccessoryView = self.toolbar;
        _textView.layer.cornerRadius = 6;
        _textView.layer.borderColor = WYOC_COLOR_HEX(0xcccccc).CGColor;
...
{
    if (!_toolbar) {
        CGRect tempFrame = CGRectMake(0, 0, WYOC_SCREEN_WIDTH, 40);
        ...
        UIBarButtonItem *bgItem = [[UIBarButtonItem alloc] initWithCustomView:({
            ...
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(25, 0, 150, 40)];
            label.numberOfLines = 0;
            label.textColor = WYOC_COLOR_HEX(0x999999);
        ...
        UIBarButtonItem *endItem = [[UIBarButtonItem alloc] initWithCustomView:({
            UIButton *end = [[UIButton alloc] initWithType:WYButtonTypeDefault normalTitle:@"完成" titleFont:14 cornerRadius:0];
